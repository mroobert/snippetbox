...
func (m *SnippetModel) Insert(title, content, expires string) (int, error) {
	...
	// 1. It creates a new prepared statement on the database using the provided SQL statement.
	// The database parses and compiles the statement, then stores it ready for execution.

	// 2. In a second separate step, Exec() passes the parameter values to the database.
	// The database then executes the prepared statement using these parameters.
	// Because the parameters are transmitted later, after the statement has been compiled, the database treats
	// them as pure data. They can’t change the intent of the statement.
	//! So long as the original statement is not derived from an untrusted data, injection cannot occur.
